"""FLUX.2 [Pro] Preview — V3 async node with 8 IMAGE slots."""
...
class Flux2ProPreview(io.ComfyNode):
    """Generate or edit images via FLUX.2 [Pro] Preview API with up to 8 reference images."""
    ...
    async def execute(
        ...
        try:
            task_id = await post_request("flux-2-pro-preview", arguments, config)
            if task_id:
                print(f"[BFL FLUX.2 Pro Preview] Task ID '{task_id}'")
            ...
            print(f"[BFL FLUX.2 Pro Preview] Error: {str(e)}")
            return io.NodeOutput(create_blank_image())
