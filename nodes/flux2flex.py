"""FLUX.2 [Flex] — V3 async node with 8 IMAGE slots, guidance & steps control."""
...
class Flux2Flex(io.ComfyNode):
    """Generate or edit images via FLUX.2 [Flex] API with up to 8 reference images."""
    ...
    async def execute(
        ...
        try:
            task_id = await post_request("flux-2-flex", arguments, config)
            if task_id:
                print(f"[BFL FLUX.2 Flex] Task ID '{task_id}'")
            ...
            print(f"[BFL FLUX.2 Flex] Error: {str(e)}")
            return io.NodeOutput(create_blank_image())
